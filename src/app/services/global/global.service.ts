...
export class GlobalService {
  ...
  async showToast(msg, duration = 3000, color, position) {
  ...
  showLoader(msg?, spinner?) {
    ...
    return this.loadingCtrl.create({
      message: msg,
      spinner: spinner ? spinner : 'crescent'
    }).then(res => {
      res.present().then(() => {
        if(!this.isLoading) {
          res.dismiss().then(() => {
  ...
  hideLoader() {
    //this.isLoading = false; using setLoader to toggle state using function
    if(this.isLoading) this.setLoader();
